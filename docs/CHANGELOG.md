...
- §4.7 — three new RPCs: `GetPlaytestParticipants(playtestId, statusFilter?)` (admin; cache-only read joining applicant + the latest `dm.sent` audit row to derive `codeSentAt` for STEAM_KEYS / AGS_CAMPAIGN rows; ADT rows return NULL `codeSentAt`); `CreateAnnouncement(playtestId, sendToFilter, subject, message)` (admin; resolves the recipient set at call time, fans `announcement_recipient` rows out + enqueues `dm_outbox` rows reusing the M2 RetryDM machinery); `ListAnnouncements(playtestId)` (admin; history with aggregated per-row `status` ∈ `{SENT, SENDING, PARTIAL, FAILED}` computed from `announcement_recipient.dm_status`).
- §5.4 — new **"Bulk announcements"** subsection covers: the `Announcement` entity, the `Send To` filter semantics (resolved at call time, NOT a stored snapshot — adding an applicant after the broadcast does NOT auto-include them), the DM-queue fan-out (one `dm_outbox` row per recipient through the same circuit-broken queue that backs approve DMs), the PII property (`subject` + `message` are NEVER in logs / metrics / audit JSONB — the `announcement.create` audit row carries IDs + counts only), the closed-playtest write block, and the indefinite retention rule. New **"Code Sent Date — derived field"** subsection covers the read-time `dm.sent` audit-row join for STEAM_KEYS / AGS_CAMPAIGN participants — no new applicant column; ADT rows have NULL Code Sent Date in M5.C and the analogue (Download Date) is deferred to M6 along with the ADT telemetry cache columns.
- §5.7 — admin pages now ship a **list + detail-page-with-tabs** shell. The Playtests list (page 1) survives unchanged; pages 2–5 collapse into a per-playtest detail page at `/playtest/<slug>` with header (breadcrumb + title + date range + status pill + Publish / Stop Playtest + share link) and four tabs (`Playtest Info` / `Distribution` / `Participants` / `Discord Bot Tools`). The `Publish` / `Stop Playtest` header verbs are pure UI copy renames over M4's existing state machine — no new transitions and no PRD §5.1 state-machine change. The pre-M5.C five-page list is retained in the doc as a historical reference cross-linked to the new tab surfaces.
- §5.9 — two new optional env vars: `ANNOUNCEMENT_MAX_SUBJECT_LEN` (default `200`) and `ANNOUNCEMENT_MAX_MESSAGE_LEN` (default `4000` — Discord DM is hard-capped at 2,000 chars per message, but the form cap is intentionally higher for editorial flexibility; the fan-out chunks at the Discord boundary). **Not added in M5.C**: `ADT_TELEMETRY_REFRESH_INTERVAL_SECONDS` (lands in M6 alongside the telemetry refresh worker).
- `schema.md` — Applicant entity gains four nullable ADT telemetry cache columns (`adtDownloadAt`, `adtTotalPlaytimeSeconds`, `adtHardwareSpecs`, `adtCrashCount`) — **all four ship dormant in M5.C**; the columns exist so M6's telemetry refresh worker + endpoint hookup lands with zero schema churn. New `announcement` table (`id`, `playtest_id`, `send_to_filter`, `subject`, `message`, `status`, `recipients_total`, `recipients_sent`, `created_by_user_id`, `created_at`) and `announcement_recipient` join (`announcement_id`, `applicant_id`, `dm_status`, `dm_sent_at`, `dm_failed_at`, `dm_error_code`). New `announcement.create` `AuditLog` action row — admin-attributed; records `{announcementId, playtestId, sendToFilter, recipientCount, createdBy}` only (`subject` + `message` are NEVER in the audit JSONB).
- `errors.md` — eleven new rows covering: banner-image URL must-be-https; platforms must be non-empty + bounded to the allowed value set; `CreateAnnouncement` empty / over-bounds subject + message with byte-exact strings (`announcement subject must not be empty`, `announcement message must not be empty`, `announcement subject must be at most 200 characters`, `announcement message must be at most 4000 characters`); `CreateAnnouncement` invalid `send_to_filter`; `CreateAnnouncement` against a closed playtest (byte-exact `playtest is closed; announcements can no longer be sent`); `GetPlaytestParticipants` invalid `status_filter`. **Not added in M5.C**: telemetry-related rows (no telemetry RPC ships).
- **No `platforms` migration**: the M1 schema already shipped `platforms TEXT[]`; the M5.C scoping doc previously framed a breaking column drop, but the migration is moot because the old single-column shape never existed in this codebase. Migration 0007 adds the announcement tables + four dormant ADT telemetry columns only.
- M5 build plan tracked in [`STATUS_M5.md`](STATUS_M5.md). Track C blocks on Track B; Track B is fully shipped (`feat/m5-b-adt-distribution`), so C1 (this scope freeze) lands and C2–C12 follow.
- **Backwards compatibility note**: every existing playtest is unaffected by the new fields (no behavior change). The detail-page-with-tabs admin surface is a pure UI re-layout — every M5.C RPC is additive; pre-M5.C admin clients (legacy modal flows) keep working against the existing `ListApplicants` / `ApproveApplicant` / `RejectApplicant` RPCs.
