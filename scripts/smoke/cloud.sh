...
declare -a m3_probes=(
    ...
    "RetryFailedDms       POST  ${BASE}/v1/admin/namespaces/${NS}/playtests/${PT}/applicants:retryFailedDms {}"
)
...
# M5.C RPC reachability gate (STATUS_M5.md C11): three new admin routes
# must reach the auth interceptor. Anything other than 401 means the
# route never registered (404) or auth regressed.
declare -a m5c_probes=(
    "GetPlaytestParticipants GET    ${BASE}/v1/admin/namespaces/${NS}/playtests/${PT}/participants               -"
    "CreateAnnouncement      POST   ${BASE}/v1/admin/namespaces/${NS}/playtests/${PT}/announcements              {\"sendToFilter\":\"ANNOUNCEMENT_SEND_TO_FILTER_ALL\",\"subject\":\"s\",\"message\":\"m\"}"
    "ListAnnouncements       GET    ${BASE}/v1/admin/namespaces/${NS}/playtests/${PT}/announcements              -"
)

for probe in "${m5c_probes[@]}"; do
    read -r name method url body <<<"$probe"
    log "M5.C ${name} requires auth (expect 401)"
    if [[ "$body" == "-" ]]; then
        code=$(curl -s -o /dev/null -w '%{http_code}' -X "$method" "$url")
    else
        code=$(curl -s -o /dev/null -w '%{http_code}' -X "$method" \
