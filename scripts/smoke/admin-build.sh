...
# OpenAPI spec + generated clients are checked in (post M3 phase 15) but
# regenerated here from the local proto-derived swagger so the smoke proves
# the codegen pipeline still works end-to-end on a clean checkout — same
# pipeline the admin codegen-fresh CI gate exercises.
log "regenerating swagger + typed clients (npm run codegen)"
npm run codegen --silent >/tmp/playtesthub-admin-codegen.log 2>&1 \
    || { tail -40 /tmp/playtesthub-admin-codegen.log >&2; fail "codegen failed"; }
